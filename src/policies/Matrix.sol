...
contract Cypher is Policy, ERC721 {
    /////////////////////////////////////////////////////////////////////////////////
    ...
    function getTotalCost(uint256 tokensPurchased_)
        public
        view
        returns (uint256 totalCost, uint256 newBasePrice)
    {
        ...
        uint256 startingPrice = basePrice > priceDecay
            ? _max(basePrice - priceDecay, RESERVE_PRICE)
            : RESERVE_PRICE;
    ...
    function purchase(uint256 tokensPurchased_, uint256 maxPrice_) external {
        ...
        if (tokensPurchased_ > currentInventory) {
            revert NotEnoughInventory();
        }

        // revert if the execution price is worse than the minPrice_
        if (totalCost > maxPrice_ * tokensPurchased_) {
            revert ExecutionPriceTooHigh();
        }
